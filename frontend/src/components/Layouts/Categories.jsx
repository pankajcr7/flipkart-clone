...
const Categories = () => {
    return (
        <>
            ...
            <section className="sm:hidden bg-white mt-16 mb-2 w-full shadow overflow-x-auto">
                <div className="flex items-center gap-1 py-3 px-2" style={{ width: 'max-content' }}>
                    {catNav.map((item, i) => (
                        <Link to={`/products?category=${item.name}`} className="flex flex-col gap-1 items-center p-2 group flex-shrink-0" key={i}>
                            <div className="h-12 w-12">
                                <img draggable="false" className="h-full w-full object-contain" src={item.icon} alt={item.name} />
                            </div>
                            <span className="text-xs text-gray-800 font-medium group-hover:text-primary-blue text-center whitespace-nowrap">{item.name}</span>
                        </Link>
                    ))}
                </div>
            </section>
